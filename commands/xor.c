...
t_b_command     *xor(char **command, t_champ *champ)
{
    ...
    byte_command->arg1 = get_arg(command[0], XOR_T_DIR_SIZE, champ);
    champ->command_size += (byte_command->arg1->size + 1);
    byte_command->arg2 = get_arg(command[1], XOR_T_DIR_SIZE,  champ);
    champ->command_size += (byte_command->arg2->size);
    byte_command->arg3 = get_arg(command[2], XOR_T_DIR_SIZE,  champ);
