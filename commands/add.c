...
t_b_command     *add(char **command, t_champ *champ)
{
    ...
    byte_command->arg1 = get_arg(command[0], ADD_T_DIR_SIZE, champ);
    champ->command_size += (byte_command->arg1->size + 1);
    byte_command->arg2 = get_arg(command[1], ADD_T_DIR_SIZE,  champ);
    champ->command_size += (byte_command->arg2->size);
    byte_command->arg3 = get_arg(command[2], ADD_T_DIR_SIZE,  champ);
