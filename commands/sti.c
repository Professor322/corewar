...
t_b_command     *sti(char **command, t_champ *champ)
{
    ...
    byte_command->arg1 = get_arg(command[0], STI_T_DIR_SIZE,  champ, byte_command);
    champ->command_size += (byte_command->arg1->size + 1);
    byte_command->arg2 = get_arg(command[1], STI_T_DIR_SIZE,  champ, byte_command);
    champ->command_size += (byte_command->arg2->size);
    byte_command->arg3 = get_arg(command[2], STI_T_DIR_SIZE, champ, byte_command);
