...
#include "../assembler.h"
#define 4 LD_T_DIR_SIZE

void        ld(t_command *command, t_foo *foo)
{
    t_arg *arg;

    arg = get_arg(command->arg1, command->position, LD_T_DIR_SIZE, foo->labels_vec);
    ...
    arg = get_arg(command->arg2, command->position, LD_T_DIR_SIZE, foo->labels_vec);
    command->position += arg->size;
