...
/*   Updated: 2019/11/23 17:20:08 by mbartole         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm-proj/includes/pque.h"
...
{
	if (!(vec = ft_vadd(vec, &((t_pque){data, priority}), sizeof(t_pque))))
		return (NULL);
	sift_up((t_pque *)(vec->cont), vec->len / sizeof(t_pque) - 1);
	return (vec);
}
...
t_pque		pop_que(t_vector *vec)
{
	...
	ft_bzero((void *)(vec->cont + vec->len), sizeof(t_pque));
	sift_down(que, 0, len);
	return (ret);
}
