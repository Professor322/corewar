...
os.system(f"TIMEFORMAT='%U %S'; (time {to_run_org}) > /dev/null 2> time_org")
os.system(f"TIMEFORMAT='%U %S'; (time {to_run_our}) > /dev/null 2> time_our")

with open('time_org', 'r') as file_org:
	raw_org = file_org.readline()
with open('time_our', 'r') as file_our:
...
print(f"{val_org:.3f} -org- {rlt} -our- {val_our:.3f}")
